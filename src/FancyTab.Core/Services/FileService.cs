...
public class FileService
{
    ...
    public FileService(IFileDialogService dialogService)
    {
    ...
    public async Task<bool> SaveAsAsync(Song song)
    {
        var filePath = await _dialogService.ShowSaveFileDialogAsync(
            SanitizeFileName(song.Title) + GtabExtension,
            FileFilter);

        if (!string.IsNullOrEmpty(filePath))
        {
            return await SaveAsync(song, filePath);
        }
    ...
    public async Task<Song?> LoadAsync(string? filePath = null)
    {
        if (string.IsNullOrEmpty(filePath))
        {
            filePath = await _dialogService.ShowOpenFileDialogAsync(FileFilter);
            if (string.IsNullOrEmpty(filePath))
            {
                return null;
            }
